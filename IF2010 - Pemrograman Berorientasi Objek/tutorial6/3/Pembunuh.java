...
public class Pembunuh implements Kelas {
    ...
    public void gunakanKemampuan(Karakter self, Karakter target) {
        if (target.isHidup()){
            int diserap = (target.getHealth() < 20 ? target.getHealth() : 20);
            System.out.println(self.getName() + " menyerap health " + target.getName() + " sebesar " + diserap);
            target.terimaSerangan(diserap);
            self.addHealth(diserap);
        } else {
