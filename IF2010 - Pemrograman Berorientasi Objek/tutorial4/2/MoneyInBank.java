public class MoneyInBank implements MoneyTransaction{
    ...
    public MoneyTransaction add(MoneyTransaction money) {
        MoneyInBank m = new MoneyInBank(getAmount() + money.getAmount(), getInterestRate(), getTotalLoanAmount());
        return m;
    }
    ...
    public MoneyTransaction subtract(MoneyTransaction money) {
        ...
        if (getAmount() > money.getAmount()){
            m = new MoneyInBank(getAmount() - money.getAmount(), getInterestRate(), getTotalLoanAmount());
        }
        else{
            m = new MoneyInBank(0, getInterestRate(), getTotalLoanAmount());
        }
        return m;
    }
    ...
    public int calcInterest() {
        return (int) (getAmount() * this.interestRate / 100);
    }
