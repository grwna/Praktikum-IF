...
public class SistemMasukSekolahListImpl implements SistemMasukSekolahList {
    ...
    public List<Student> getStudentsWithHighGpa(double minGpa) {
        ...
        List<Student> high = new LinkedList<>();
        for (int i = 0; i < students.size(); i++){
            if (students.get(i).getGpa() >= minGpa){
                high.add(students.get(i));
            }
        }
        return high;
    }
    ...
    public List<String> getYoungHighAchieverNames(int maxAge, double minGpa) {
        ...
        List<String> high = new LinkedList<>();
        for (int i = 0; i < students.size(); i++){
            if (students.get(i).getAge() < maxAge && students.get(i).getGpa() >= minGpa){
                high.add(students.get(i).getName());
            }
        }
        return high;
    }
